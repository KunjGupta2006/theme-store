...
export async function createProduct(_prevState: ActionResult, formData: FormData): Promise<ActionResult> {
  try {
    ...
    await db.productVariant.createMany({
      data: sizes.flatMap((size) =>
        colors.map((color) => ({ productId: product.id, size, color, stockQuantity: 0, priceAdjustment: 0 }))
      ),
    });
  ...
  redirect("/admin/products"); // outside the try/catch — redirect() must never be swallowed by a catch block
}
