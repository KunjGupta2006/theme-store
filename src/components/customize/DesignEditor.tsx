...
export function DesignEditor({ product , initialColor, initialSize, pricing }: DesignEditorProps) {
  ...
  async function handleFinishDesign() {
    ...
    if (elements.length === 0) { setError("Please add a design to at least one side"); return; }
  ...
  async function handleSaveDraft() {
    if (elements.length === 0) { setError("Add a design element before saving a draft"); return; }
    setError(null);
    setSavingDraft(true);
    try {
      const [frontUrl, backUrl] = await Promise.all([exportSide("front"), exportSide("back")]);
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#F5F1EA]">
      {/* Left — Studio Editor */}
      <div className="w-72 bg-[#FAF7F2] border-r border-black/6 flex flex-col shrink-0">
