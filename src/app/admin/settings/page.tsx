...
export default async function SettingsPage() {
  const settings = await db.storeSettings.upsert({
    where: { id: "singleton" },
    update: {},
    create: { id: "singleton", customShirtBasePrice: 150, printChargePerSide: 15, shippingFlatRate: 79, freeShippingThreshold: 999 },
  });
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-sm text-[#666] mt-1">Configure global store pricing and shipping rates.</p>
      </div>

      <div className="bg-[#FAF7F2] border border-black/[0.06] rounded overflow-hidden">
        <div className="px-6 py-4 border-b border-black/[0.06]">
          <h2 className="text-sm font-medium text-[#111111]">Global Settings</h2>
        </div>
