...
export default async function AdminUsersPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-['Inter_Tight'] text-3xl font-bold text-[#111111]">Users</h1>
        ...
        <table className="w-full">
          ...
          <tbody>
            {users.length === 0 ? (
              ...
              users.map((user) => {
                const toggleRole = updateUserRole.bind(null, user.id);
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-[#666666]">{user._count.orders}</td>
                    <td className="px-6 py-4 text-xs text-[#666666]">
                      {user.createdAt.toLocaleDateString("en-IN")}
                    </td>
                    <td className="px-6 py-4">
                      <RoleToggleForm action={toggleRole} userName={user.name} nextRole={nextRole} />
                    </td>
                  </tr>
                );
              })
